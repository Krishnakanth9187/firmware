...
double messageCb(const geometry_msgs::Twist& msg) 
{
  ...
  if(m < 2){
        ...
        vel[m][0] =  max(min(msg.linear.x, 1.0f), -1.0f);
        vel[m][1] = max(min(msg.angular.z, 1.0f), -1.0f);
    }
    else{
      m = -1;
    }
...
double pid(double v,double u){
 ...
 // }
  ...
  PWM = mapFloat(PWM, 0.0, 0.7, out_min, out_max);
...
double pid1(double l,double m){
 ...
 // }
  ...
  PWM1 = mapFloat(PWM1, 0.0, 0.7, out_min, out_max);
